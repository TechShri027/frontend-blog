...
export default function BlogList({ user, token }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {blogs.map(blog => (
          <div key={blog._id} className="bg-white rounded-xl shadow-md p-6 hover:shadow-xl transition">
            ...
            <div className="flex items-center justify-between">
              <button onClick={() => likeBlog(blog._id)} className="flex items-center gap-1 text-red-500 hover:text-red-600 transition">
                ❤️ <span>{blog.likes}</span>
              </button>

              <Link to={`/blog/${blog._id}`} className="text-blue-600 hover:text-blue-700 text-sm font-medium">
                Read More →
              </Link>
            </div>

            {user?.role === "admin" && (
              <button onClick={() => handleDelete(blog._id)} className="mt-4 bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">
                Delete
              </button>
            )}
          </div>
      ...
      {blogs.length === 0 && <p className="text-gray-500 mt-10 text-center">No blogs available yet.</p>}
    </div>
  );
}
